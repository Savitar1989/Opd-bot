...
WEBAPP_URL = "https://8703adbb39ac.ngrok-free.app"
...
class RestaurantBot:
    ...
    async def process_notifications(self, context: ContextTypes.DEFAULT_TYPE):
        ...
        while processed_count < max_per_batch:
            try: item = notification_queue.get_nowait(); processed_count += 1
            except Empty: break
            max_retries = 3
            for attempt in range(max_retries):
                try:
                    ...
                    logger.error(f"Failed to send notification (attempt {attempt+1}): {e}")
...
<body>
  ...
  <form action="/admin/export_excel" method="get" style="margin:10px 0;">
    <input type="hidden" name="init_data" id="export-init-data">
    <button type="submit">📊 Export Excel</button>
  </form>

  <script>
    document.getElementById("export-init-data").value =
  ...
  {% for courier, weeks in courier_stats.items() %}
    <div class="courier-block">
      ...
      {% for r in weeks %}
        ...
        {% set weeklabel = year ~ ". hét " ~ weeknum %}
        ...
        <div class="week-content" style="display: none;">
          <table border="1" style="width: 100%; margin-top: 10px;">
            <tr><th>Hét</th><th>Darab</th><th>Átlag idő (perc)</th></tr>
            <tr>
              <td>{{ r.week }}</td>
              <td>{{ r.cnt }}</td>
              <td>{{ r.avg_min }}</td>
