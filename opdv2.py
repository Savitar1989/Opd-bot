...
class RestaurantBot:
    ...
    def _setup_handlers(self) -> None:
        ...
        # JAVÍTÁS: Gyakoribb értesítés ellenőrzés és retry mechanizmus
        if app.job_queue:
            app.job_queue.run_repeating(self.process_notifications, interval=2)  # 2 másodpercenként
...
class RestaurantBot:
    ...
    async def process_notifications(self, context: ContextTypes.DEFAULT_TYPE):
        """JAVÍTOTT értesítés feldolgozás retry logikával"""
        processed_count = 0
        max_per_batch = 5  # Maximum 5 értesítés batch-enként
        ...
        while processed_count < max_per_batch:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    logger.info(f"Notification sent successfully to {item['chat_id']}")
                    break  # Sikeres küldés, kilép a retry ciklusból
