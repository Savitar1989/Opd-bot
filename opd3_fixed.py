...
class DatabaseManager:
    ...
    def get_orders_by_courier(self, courier_id, status_filter=None):
        ...
        if status_filter:
            cur.execute("""
                SELECT id, restaurant_name, restaurant_address, phone_number, order_details, group_id, group_name, created_at, status, estimated_time
                FROM orders
...
async function wazeLink(orderId){
  ...
  async function openGoogleMaps(orderId) {
      ...
      if (link !== '#') {
        if (typeof tg !== 'undefined' && tg && tg.openLink) tg.openLink(link);
